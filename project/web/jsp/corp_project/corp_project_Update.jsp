...
$( function() {
	...
	$("#projectSaveBtn").on("click",function(){
		...
		$.ajax({
    		url: "ProjectServlet?command=corp_project_update"
    	     , type:"post"
    	     , data:{'skills':skills
    	    	    ,'empList':empList
    	    	    ,'emp_roles':emp_roles
					...
					<table>
						<tbody>
							<tr>
								...
								<td><input type="text" value=" ${corpProject.project_name}" name="project_name" id="project_name" class="input_type5" maxlength="30"   placeholder="프로젝트 이름을 입력해주세요 "></td>
							</tr>
							<tr>
								<th><span>회사 이름</span></th>
								<td><input type="text" value=" ${corpProject.corp_name}" name="corp_name" id="corp_name" class="input_type4"   maxlength="20"   placeholder="회사 이름을 입력해주세요"></td>
							</tr>
							<tr>
								<th><span>회사 구분</span></th>
								<td>
									<select id="corp_own">
										<c:if test="${corpProject.corp_own=='내부' }">
											<option value="내부" selected="selected">내부</option>
										</c:if>
										<c:if test="${corpProject.corp_own=='외부' }">
							...
							<tr>
								...
								<td><input type="text" value=" ${corpProject.start_dt}" name="start_dt" id="start_dt" class="input_type2" placeholder="프로젝트 시작일"></td>
							</tr>
							<tr>
								<th><span>종료 일</span></th>
								<td><input type="text" value=" ${corpProject.end_dt}" name="end_dt" id="end_dt" class="input_type2" placeholder="프로젝트 종료일"></td>
							</tr>
							<tr>
								<th><span>프로젝트 내용</span></th>
								<td>
									<textarea name="project_content" id="project_content" cols="120" rows="15">${corpProject.project_content}</textarea>
								</td>
							</tr>
							<tr>
									...
									<c:forEach var="skill" items="${skillList}" varStatus="status">
											<span class="skillArea">
												<c:if test="${skill.isChecked =='y'}">
													<input type="checkbox" id="skills" name="skills" checked="checked" value="${skill.skill_id}"/>${skill.skill_name}
												</c:if>
												<c:if test="${skill.isChecked=='n'}">
							...
							<tr>
								<th><span>비고</span></th>
								<td><input type="text" value=" ${corpProject.remarks}" name="remarks" id="remarks" class="input_type3"></td>
							</tr>
						</tbody>
					</table>
				</div>
							...
							<tbody class="eList">
								<c:forEach var="emp" items="${empList}">
									<tr>
										...
										<td>
											<select>
											<c:forEach var="role" items="${empRole}">
												<c:if test="${role.role_id==emp.role_id}">
